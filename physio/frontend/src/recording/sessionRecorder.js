...
export function createSessionRecorder(sessionId, exerciseId) {
  ...
  function recordPacket(packet, smoothedFrame = null) {
    ...
    const sample = {
      ...
      jitter_event: packet?.jitter_grouped_event ?? packet?.jitter_event ?? smoothedFrame?.jitter?.jitterGroupedEvent ?? false,
      jitter_grouped_event: packet?.jitter_grouped_event ?? packet?.jitter_event ?? smoothedFrame?.jitter?.jitterGroupedEvent ?? false,
      jitter_reason: packet?.jitter_reason ?? smoothedFrame?.jitter?.jitterReason ?? null,
      predicted_trend_angle: packet?.predicted_trend_angle ?? smoothedFrame?.jitter?.predictedTrendAngle ?? null,
      trend_residual: packet?.trend_residual ?? smoothedFrame?.jitter?.trendResidual ?? null,
      jitter_debug: packet?.jitter_debug ?? smoothedFrame?.jitter?.debug ?? null,
